...
function isAllowedHostname(hostname: string) {
  ...
  return (
    isSingleLabelHostname(normalizedHostname) ||
...
export function validateAndNormalizeServerUrl(rawServerUrl: string) {
  let parsedUrl: URL;
  ...
  if (!parsedUrl.hostname || !isAllowedHostname(parsedUrl.hostname)) {
    throw new Error(
      "Actual server URL host is not allowed. Set ACTUAL_ALLOWED_HOSTS to permit a specific host.",
    );
  }
