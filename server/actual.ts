...
async function ensureInitialized() {
  if (!hasStarted) {
    const dataDir = process.env.ACTUAL_DATA_DIR ?? path.join(os.tmpdir(), 'actual-budget-ofx');
...
export async function getTransactionsForMonth(accountId: string, month: string) {
  await ensureInitialized();
