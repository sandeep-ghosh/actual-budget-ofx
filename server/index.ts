...
import { connectToActual, getAccounts, getBudgetMonths, getTransactionsForMonth } from './actual';
...
app.post('/api/export-ofx', async (req, res) => {
  ...
  if (typeof accountId !== 'string' || !accountId.trim() || typeof month !== 'string' || !month.trim()) {
    return res.status(400).json({ error: 'accountId and month are required (format: YYYY-MM)' });
  }

  try {
    const accounts = await getAccounts();
    const account = accounts.find((a: any) => a.id === accountId) || accounts.find((a: any) => String(a.id) === String(accountId));
    ...
    return res.status(500).json({ error: error?.message ?? 'Failed to generate OFX' });
  }
});
